...
import { estaCompletado } from "../progreso/progreso.js";
import { cargarNiveles, obtenerNiveles } from "./datosNiveles.js";

let nivelActualId = 1;
let animando = false;
let elementos = [];
let alComenzarNivel = null;

export function establecerAlComenzarNivel(callback) {
  alComenzarNivel = callback;
}
...
function nivelDesbloqueado(id) {
  return id === 1 || estaCompletado(id - 1);
}
...
function llenarTarjeta(elemento, nivel) {
  ...
  img.src = `assets/imagenes/niveles/nivel${nivel.id}.png?v=${Date.now()}`;
...
function cambiarNivel(direccion) {
  if (animando || obtenerNiveles().length === 0) return;
  animando = true;
...
function irANivel(numeroDeseado) {
  ...
  if (!Number.isFinite(numero) || numero < 1 || numero > total) {
    document.getElementById("input-ir-a-nivel").value = "";
    return;
  }
  ...
  const tarjetaCentro = elementos[1];
  tarjetaCentro.classList.remove("tarjeta-nivel--destello");
  void tarjetaCentro.offsetWidth;
  tarjetaCentro.classList.add("tarjeta-nivel--destello");
}
...
function manejarClicComenzar(evento) {
  ...
  alComenzarNivel?.(nivelActualId);
}

export async function inicializarMapaNiveles() {
  await cargarNiveles();
  ...
  document.getElementById("flecha-izquierda").addEventListener("click", () => cambiarNivel(-1));
  document.getElementById("flecha-derecha").addEventListener("click", () => cambiarNivel(1));
  pista.addEventListener("click", manejarClicComenzar);

  document.getElementById("form-ir-a-nivel").addEventListener("submit", (evento) => {
    evento.preventDefault();
    const valor = document.getElementById("input-ir-a-nivel").value;
    irANivel(Number(valor));
  });
}

export async function mostrarMapaNiveles() {
  await cargarNiveles();
  renderizarTarjetas();
  mostrarPantalla("niveles");
}

export function refrescarMapaNiveles() {
  renderizarTarjetas();
}

export function irANivelEspecifico(id) {
  nivelActualId = id;
  renderizarTarjetas();
}